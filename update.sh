...
# Check if both arguments are provided (unless --home-only is specified)
if [ -z "$home_config" ] || { [ -z "$system_config" ] && [ "$flags" != "--home-only" ]; }; then
    echo "Usage: $0 <home_config> <system_config> [--verbose] [--rebuild-bootloader] [--home-only]"
    echo "Available system configurations: $system_configs"
    echo "Available home configurations: $home_configs"
    exit 1
fi

# Check if the chosen configurations are in the lists
...
if [ "$flags" != "--home-only" ] && ! echo $system_configs | grep -q $system_config; then
    echo "Invalid system configuration: $system_config"
    echo "Available system configurations: $system_configs"
    exit 1
fi

# If --home-only is not specified, proceed with system configuration changes
if [ "$flags" != "--home-only" ]; then
